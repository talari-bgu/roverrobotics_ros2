...
class NavigateThroughPosesNode(Node):
    def __init__(self):
        ...
        # Initialize TF listener for tracking robot's position
        self.tf_buffer = Buffer()
        self.tf_listener = TransformListener(self.tf_buffer, self)

        self.robot_trajectory = []  # Store robot positions
        self.create_timer(1.0, self.record_robot_position)  # Record position every 0.1 seconds
    ...
    def record_robot_position(self):
        try:
            ...
            if self.tf_buffer.can_transform('map', 'base_link', rclpy.time.Time(), rclpy.duration.Duration(seconds=1.0)):
            ...
            self.get_logger().warn(f"Could not record position: {e}")

    def save_trajectory(self):
        directory = "saved_trajectories"
        os.makedirs(directory, exist_ok=True)

        # Generate file name based on input JSON file name
        file_name = "2f.20_trajectory.csv"  # Replace with dynamic name if needed
